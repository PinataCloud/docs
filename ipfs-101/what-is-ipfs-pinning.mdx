...
description: "IPFS pinning keeps a file available on the network by telling at least one node to hold it. Learn what pinning is and why services like Pinata exist."
...
We also have other tools like the [Pinata CLI](/tools/cli/ipfs) or [Next.js Starter](/tools/nextjs-starter) which can be used to upload using [API Keys](/account-management/api-keys).
