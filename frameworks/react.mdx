...
It is still highly recommend using the [Next.js Quickstart](/frameworks/next-js) as it is much more secure with server side API routes and works similar to React.
...
```typescript src/App.tsx - Folders
import { useState } from "react";
...
After we have uploaded the file and get the CID back we can create a URL for the file to either download it or render it in our app. For this example we will assume the file is an image, but Pinata supports any kind of file type.
...
function App() {
  ...
  const handleSubmission = async () => {
    try {
      const upload = await pinata.upload.public.file(selectedFile)
      console.log(upload);
      const gatewayUrl = await pinata.gateways.public.covert(upload.cid)
      setUrl(gatewayUrl)
    } catch (error) {
      console.log(error);
    }
