...
To list all the files that are part of a group you can send a `GET` request to `/data/pinList` with the query parameter of `groupId`. This will return the `pinList` object.
...
<Note>
  ...
  Please use [/pinning/unpin](/api-reference/endpoint/unpin-file) to actually
