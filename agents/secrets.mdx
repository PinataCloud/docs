...
| **OpenRouter** | API key |
| **Pinata** | [Pinata-hosted inference](/inference/overview) — no key needed |
...
<Note>
  The **Pinata** provider is first-party — connecting it just enables [Pinata-hosted inference](/inference/overview) for your account, no API key required. You can also toggle it programmatically with the enable/disable endpoints (see [Inference → Enable](/inference/overview#enable-inference) and [Disable](/inference/overview#disable-inference)).
</Note>
