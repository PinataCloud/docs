...
This is the first thing you'll do. Without an LLM key, your agent can't think.

Go to [agents.pinata.cloud/secrets](https://agents.pinata.cloud/secrets). At the top you'll see cards for Anthropic, OpenAI, and OpenRouter. Click **Connect** on whichever you use, paste your API key, and save.

That's the minimum you need to run an agent.

<Tip>
  If you're using OpenAI with a Codex subscription, you can connect via OAuth instead. Run `pinata agents auth openai --oauth` in the [CLI](/tools/cli/overview) and follow the browser flow.
</Tip>
...
Click **Add Variable** instead of **Add Secret** if you don't need encryption.
...
**For existing agents:** Open the agent → **Secrets** → add from your vault.
...
- Updated secrets require an agent restart
