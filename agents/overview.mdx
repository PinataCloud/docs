...
Pinata Agents gives you a hosted AI agent in its own sandboxed container. The agent has a workspace it can read and write to, a terminal it can run commands in, and connectors for the outside world — chat apps, web servers, scheduled jobs. You talk to it through the dashboard, the CLI, or whichever messaging platform you connect it to. The current dashboard also includes a built-in copilot ("Pinnie"), a standalone AI model catalog, and an issue board for assigning work to agents.

Each agent runs on an **Agent Engine** — `openclaw` (default) for full configurability, `hermes` for an opinionated smoother out-of-the-box experience, or other enabled engines such as `superbuilder` for issue-board coding workflows. You pick one when you create the agent. The default OpenClaw engine is documented at [openclaw.org](https://openclaw.org); for the differences see [Concepts → Engine](/agents/concepts#engine).
...
Your agent needs an LLM to think. Go to the [Secrets Vault](https://agents.pinata.cloud/secrets), open **AI Models** in the dashboard, or just start the create flow — all three surfaces can connect a provider key.

In the current UI you'll see provider cards for **Pinata**, **OpenAI**, **Anthropic**, **OpenRouter**, **Venice**, and **Custom**. Click **Connect** on whichever one you use, paste your API key (or run through OAuth if you're using an OpenAI Codex subscription), and save.
...
- **Build from scratch** — full control. The current wizard is a two-step `Setup -> Deploy` flow. You choose an **Agent Engine** (OpenClaw, Hermes, Super Builder, or another enabled engine), connect an AI provider or use Pinata's free tier, name the agent, and optionally expand **Advanced options** for personality, channels, variables/secrets, and skills before deploying.

Either way, the agent takes about 30 seconds to provision. When it's ready you land on its Chat tab.

<Frame>
  ![Current create wizard](/images/agents-create-advanced-2026-06-10.png)
</Frame>
...
<Frame>
  ![Current My Agents landing page](/images/agents-home-2026-06-10.png)
</Frame>
...
| **My Issues** | A kanban-style issue board for agent work. Backlog, In progress, Review, and Done columns let you queue and review runs. |
...
| **AI Models** | A standalone model catalog where you can browse providers, star models, and run direct BYOK test chats without creating an agent. |
| **Marketplace** | Published templates from Pinata and other builders, ready to deploy in one click. |
| **My Templates** | Still available, but currently nested under **Marketplace** in the sidebar. |
...
<CardGroup cols={2}>
  ...
  <Card title="Issues" icon="list-check" href="/agents/issues">
    Assign work, review runs, and track agent tasks on the board
  </Card>
