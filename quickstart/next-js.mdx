...
<Steps>
  <Step title="Create an API Key and get Gateway URL">
  ...
  <Step title="Start up Next.js Project">
...
<Note>Next.js does have a file size limitation for what can be passed through the API routes, so if you need more than the limit then it is advised to make signed JWTs by folloing [this guide](https://www.pinata.cloud/blog/how-to-upload-to-ipfs-from-the-frontend-with-signed-jwts).</Note>
...
This will accept a `POST` request from the client, create [`pinataMetadata`](/pinning/pinata-metadata), then send an API request to Pinata with the upload. Once complete it will return the [`CID`](/ipfs-101/what-are-cids) to the client.
  ...
  <Step title="Pull Content from IPFS">
...
export default function Home() {
  ...
  return (
    <main className="w-full min-h-screen m-auto flex flex-col justify-center items-center">
      ...
      {cid && <img src={`${process.env.NEXT_PUBLIC_GATEWAY_URL}/ipfs/${cid}`} alt="Image from IPFS" />}
    </main>
  );
}
```
