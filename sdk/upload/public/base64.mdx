---
title: "upload.public.base64"
description: "Pinata SDK upload.public.base64 uploads a base64-encoded payload to public IPFS, useful for server actions that already encode binary data."
---

Upload a base64 string to Pinata

## Usage

```typescript
import { PinataSDK } from "pinata";

const pinata = new PinataSDK({
  pinataJwt: process.env.PINATA_JWT!,
  pinataGateway: "example-gateway.mypinata.cloud",
});

const upload = await pinata.upload.public.base64("SGVsbG8gV29ybGQh")
```

## Returns

```typescript
type UploadResponse = {
	id: string;
	name: string;
	cid: string;
	size: number;
	created_at: string;
	number_of_files: number;
	mime_type: string;
	group_id: string | null;
	keyvalues: {
		[key: string]: string;
	};
	vectorized: boolean;
	network: string;
};
```

## Parameters

### base64

- Type: `string`

Accepts a string encoded in base64

```typescript
const upload = await pinata.upload.public.base64("SGVsbG8gV29ybGQh")
```

### group (Optional)

- Type: `string`

Upload to a specific group by passing in the `groupId`

```typescript {3}
const upload = await pinata.upload.public
  .base64("SGVsbG8gV29ybGQh")
  .group("b07da1ff-efa4-49af-bdea-9d95d8881103")
```

### keyvalues (Optional)

- Type: `Record<string, string>`

Add optional keyvalues to file

```typescript {3-8}
const upload = await pinata.upload.public
  .base64("SGVsbG8gV29ybGQh")
  .keyvalues({
    env: "prod"
  })
```

### name (Optional)

- Type: `string`

Add optional name to file

```typescript {3-8}
const upload = await pinata.upload.public
  .base64("SGVsbG8gV29ybGQh")
  .name("hello.txt")
```

### url (Optional)

- Type: `string`

Pass in a presigned upload URL created with [createSignedURL](./create-signed-url)

```typescript {3}
const upload = await pinata.upload.public
  .file(file)
  .url(url)
```

### key (Optional)

- Type: `string`

Upload a file using a secondary API key generated through `keys.create()`

```typescript {3}
const upload = await pinata.upload.public
  .base64("SGVsbG8gV29ybGQh")
  .key("GENERATED_API_JWT")
```
