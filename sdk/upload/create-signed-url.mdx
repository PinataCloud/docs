...
Create a Signed Upload URL for a file upload. This method is ideal when you want to authorize server side but upload on the client.
...
const signedURL = await pinata.upload.createSignedURL({
  expires: 30
});

const upload = await pinata.upload.file(file).url(signedURL)
```
