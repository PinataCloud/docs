...
Private IPFS gives you the ability to query uploaded files based on different filters and attributes such as name, [key-values](/files/key-values), date, and more. This is different from retrieving the actual contents of a file, which you can learn more about [here](/gateways/retrieving-files).

## Basic Usage

You can either use the [Private IPFS SDK](/sdk/data/list) or the [Private IPFS API](/api-reference/endpoint/list-files) as see in the examples below.
...
```typescript API
const url = "https://api.pinata.cloud/v3/files?metadata[env]=prod"
```
