...
```
https://402.pinata.cloud/v1
```

### `POST /pin/:network`

Pins a file to either Public or Private IPFS
...
<ParamField path="network" type="public | private" required>
  Use either `public` or `private` IPFS
</ParamField>

**Request Body**

<ParamField body="fileSize" type="integer" required>
  The size of the file you want to upload in bytes. If the file exceeds the size sent the file upload will fail
</ParamField>

**Returns**

<ResponseField name="url" type="string">
  A [presigned URL](/files/presigned-urls) that can be used to upload the file
</ResponseField>
...
<ParamField path="cid" type="string" required>
  CID of the file on [Private IPFS](/files/private-ipfs) you are trying to access
</ParamField>

**Returns**

<ResponseField name="url" type="string">
  A [temporary access URL](/files/private-ipfs#accessing-private-files) that can be used to access the file for a limited amount of time
</ResponseField>
...
fetchWithPayment(url, {
	...
	.then(async (response) => {
		...
		const file = new File(
			[`Paid and pinned by 402.pinata.cloud: ${uuid}`],
			"file.txt",
		);
