...
CAR file processing is **asynchronous**. After a successful CAR file upload, Pinata will validate the CAR file structure and IPFS blocks and prepare the file for indexing into the IPFS network. A notifcation will be sent via webhook to indicate wether the CAR file was successfully indexed and available on the network. 
