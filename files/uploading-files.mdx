...
In addition to our IPFS product is our new Private IPFS API. It works just like your normal IPFS uploads where you get a CID in return, except these files can only be accessed through your Dedicated Gateway with a time sensitive access token. This means these files are not broadcasted to the public IPFS network and stay strictly with Pinata. It comes in handy if you want to token gate content that would otherwise be exposed on IPFS.
...
The Private IPFS API upload endpoint `https://uploads.pinata.cloud/v3/files` is fully [TUS](https://tus.io) compatible, so it can support larger files with the ability to resume uploads. Any file upload larger than 100MB needs to be uploaded through the TUS method. The [Private IPFS SDK](/sdk) handles this automatically when you use `pinata.upload.file()` by checking the file size before uploading.
