...
- Pinata has an [SDK method](/sdk/files/public/swap-history) and [API endpoint](/api-reference/endpoint/get-swap-history) that any Pinata user can call to check the history of a swap
...
async function main() {
  try {
    ...
    const swap = await pinata.files.public.swapCid({
      cid: CID1,
      swapCid: CID2,
    });
...
```typescript
const history = await pinata.files.public.swapHistory({
  cid: "bafkreibbvdqf5ekc2crouowv7vtjbcmbjoiysegslmmqc6jrxbaa43xske",
